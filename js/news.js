/* =========================================================
   ...
   データは js/news-data.js（サイト側で書く分）と、
   スプレッドシート［お知らせ］（コーチが書く分・sheet-data.js が読み込んで追加）
   ========================================================= */
(function () {
  'use strict';
  ...
  function render(posts) {
    ...
    if (article) {
      ...
      if (idx < 0) {
        // スプレッドシート分がまだ読み込み中かもしれないので、最終確定までは控えめに
  ...
  var base = (window.NEWS_POSTS || []).slice();
  render(base);

  // スプレッドシートのお知らせが読み込めたら、サイト側の分と合わせて描き直す
  window.NORTE_NEWS = {
    addSheetPosts: function (extra) {
      if (!extra || !extra.length) return;
      render(base.concat(extra));
    }
  };
})();
